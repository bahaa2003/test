...
export const validateDepartment = (req, res, next) => {
  const schema = Joi.object({
    ...
    code: Joi.string().uppercase().trim().pattern(/^[A-Z]{2,4}$/).required()
      .messages({
    ...
    headOfDepartment: Joi.string().hex().length(24).optional(),
    totalYears: Joi.number().integer().min(1).max(6).default(4)
      .messages({
        'number.min': 'يجب أن يكون للقسم سنة دراسية واحدة على الأقل',
        'number.max': 'لا يمكن أن يزيد عدد السنوات عن 6'
      }),
    studySystem: Joi.string().valid('semester', 'trimester', 'quarter').default('semester')
  });
